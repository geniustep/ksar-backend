...
نظام متكامل لإدارة طلبات المساعدة الإنسانية، يُمكّن المواطنين من تقديم طلباتهم بسهولة، ويُتيح للمؤسسات الخيرية التكفل بها ومتابعتها حتى الإتمام.
...
| الميزة | الوصف |
|--------|-------|
| 🌐 **تقديم طلبات بدون تسجيل** | يمكن للمواطنين تقديم طلباتهم مباشرة |
...
│  يقدم طلبه     │     │  تراقب وتوجه   │     │ تتكفل وتُنجز   │
└─────────────────┘     └─────────────────┘     └─────────────────┘
        │                       │                       │
        ▼                       ▼                       ▼
   [طلب جديد]           [مراقبة + تحليل]        [تكفل + إتمام]
```
...
#### متابعة الطلب
```http
GET /api/v1/public/requests/track?code=ABC12345&phone=0612345678
```
...
├──────────────┤     ├──────────────┤     ├──────────────┤
│ id           │     │ id           │     │ id           │
│ email        │     │ tracking_code│     │ name         │
│ password_hash│     │ requester_*  │     │ user_id (FK) │
│ full_name    │     │ category     │     │ contact_*    │
│ role         │     │ status       │     │ is_active    │
│ org_id (FK)  │     │ priority     │     │ total_done   │
└──────────────┘     └──────────────┘     └──────────────┘
                            │
                            ▼
                     ┌──────────────┐
